...
class OrderItemsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index should return unauthorized when token is absent" do
    ...
    assert_response 401
    assert response.parsed_body['errors'].include?(ERROR_MESSAGES[:nil_token])
  end
  ...
  test "create should return forbidden if current user is chef" do
    ...
    assert_response 403
    assert response.parsed_body['errors'].include?(ERROR_MESSAGES[:not_privileged])
  end
  ...
  test "show should return not_found if order_id is invalid" do
    ...
    assert_response 404
    assert response.parsed_body['errors'].include?(ORDER_NOT_FOUND)
  end
  ...
  test "show should return not_found if order_item_id is invalid" do
    ...
    assert_response 404
    assert response.parsed_body['errors'].include?(ORDER_ITEM_NOT_FOUND)
  end

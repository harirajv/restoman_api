...
class UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get index" do
    get users_url, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
    assert_equal @user.account.users.map(&:facade).as_json, response.parsed_body
  end
  ...
  test "create should return forbidden if user does not have privilege" do
    ...
    post users_url, params: { name: @user.name, role: @user.role, email: 'new@email.com', password: 'password' }, headers: { 'Authorization': JsonWebToken.generate_token(not_admin) }, as: :json
    assert_response 403
  ...
  test "create should return bad_request if required model parameters are not present" do
    post users_url, params: { invalid: true }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
  ...
  test "should create user" do
    ...
    assert_emails 1 do
      assert_difference('User.count') do
        post users_url, params: payload, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
      end
    end
  ...
  test "show should return not_found if user_id is invalid" do
    get user_url(0), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end

  test "should show user" do
    get user_url(@user), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
  ...
  test "update should return forbidden if current user is not admin" do
    not_admin = users(:waiter)
    put user_url(not_admin), params: { role: not_admin.role }, headers: { 'Authorization': JsonWebToken.generate_token(not_admin) }, as: :json
    assert_response 403
    assert_json_match(error_response(ERROR_MESSAGES[:not_privileged]), response.body)
  ...
  test "update should return bad_request if parameters are empty" do
    put user_url(@user), params: { invalid: true }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
  ...
  test "update should return bad_request if required model parameters are not present" do
    put user_url(@user), params: { something: true }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
    assert_json_match(error_response(ERROR_MESSAGES[:missing_params]), response.body)
  end
  ...
  test "update should return not_found if user_id is invalid" do
    put user_url(0), params: { name: @user.name }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end
  ...
  test "should update user" do
    ...
    put user_url(@user), params: payload, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
    @user.reload
    pattern = payload.merge(
      id: @user.id,

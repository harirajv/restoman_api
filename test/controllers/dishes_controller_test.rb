...
class DishesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index should return dishes" do
    get dishes_url, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
    assert_json_match(@user.account.dishes.map(&:as_json), response.body)
  end

  test "forbidden returned by create if not privileged" do
    @user = users(:waiter)
    post dishes_url, params: { cost: @dish.cost, description: @dish.description, image: @dish.image, name: @dish.name }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 403
  ...
  test "create should return bad_request if required model parameters are not present" do
    post dishes_url, params: { invalid: true }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
  ...
  test "create should return bad_request if unpermitted model parameters are present" do
    post dishes_url, params: { created_at: Time.now }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
  ...
  test "should create dish" do
    ...
    assert_difference('Dish.count') do
      post dishes_url, params: payload, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    end

    assert_response 201
    pattern = payload.merge({
      id: wildcard_matcher,
  ...
  test "not_found returned by show for invalid id" do
    get dish_url(-1), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end

  test "should show dish" do
    get dish_url(@dish), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
  ...
  test "forbidden returned by update if restricted action for current_user" do
    @user = users(:waiter)
    put dish_url(@dish), params: { cost: @dish.cost, description: @dish.description, image: @dish.image, name: @dish.name, is_active: false }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 403
    assert_json_match(error_response(ERROR_MESSAGES[:not_privileged]), response.body)
  end
  ...
  test "not_found returned by update for invalid id" do
    @new_dish = dishes(:two)
    put dish_url(-1), params: { cost: @new_dish.cost, description: @new_dish.description, image: @new_dish.image, name: @new_dish.name, is_active: false }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end

  test "update should return bad_request if required model parameters are not present" do
    put dish_url(@dish), params: { invalid: true }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
    assert_equal ERROR_MESSAGES[:missing_params], response.parsed_body['errors'].first
  end

  test "update should return bad_request if unpermitted model parameters are present" do
    put dish_url(@dish), params: { created_at: Time.now }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 400
    assert_equal ERROR_MESSAGES[:invalid_params] % 'created_at', response.parsed_body['errors'].first
  end
  ...
  test "should update dish" do
    ...
    put dish_url(@dish), params: payload, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
    @dish.reload
    pattern = payload.merge({
      id: @dish.id,
      account_id: @dish.account_id,
      created_at: @dish.created_at.as_json,
      updated_at: @dish.updated_at.as_json
    })
    assert_json_match(pattern, response.body)
  end
  ...
  test "forbidden returned by destroy if restricted action for current_user" do
    @user = users(:waiter)
    delete dish_url(@dish), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 403
    assert_json_match(error_response(ERROR_MESSAGES[:not_privileged]), response.body)
  ...
  test "not_found returned by destroy for invalid id" do
    delete dish_url(-1), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end

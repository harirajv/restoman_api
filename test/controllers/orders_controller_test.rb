...
class OrdersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index should return orders with order items" do
    get orders_url, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json

    assert_response 200
    assert_equal @user.account.orders.includes(:order_items).to_json(include: :order_items), response.body
  end
  ...
  test "create should return forbidden if current user is chef" do
    chef = users(:chef)
    post orders_url, params: { table_no: @order.table_no }, headers: { 'Authorization': JsonWebToken.generate_token(chef) }, as: :json
  ...
  #   post orders_url, params: { invalid: true }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
  #   assert_response 422
  ...
  #   post orders_url, params: { created_at: Time.now }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
  #   assert_response 422
  ...
  test "should create order with order_items" do
    ...
    assert_difference ->{ Order.count } => 1, -> { OrderItem.count } => 2 do
      post orders_url, params: payload, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    end
  ...
  test "show should return not_found if order_id is invalid" do
    get order_url(0), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end

  test "should show order" do
    get order_url(@order), headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 200
  ...
  test "update should return forbidden if current_user is chef and status parameter is other than completed" do
    chef = users(:chef)
    put order_url(@order), params: { order_items: [{ id: @order.order_items.first.id, status: :preparing }] }, headers: { Authorization: JsonWebToken.generate_token(chef) }, as: :json
    assert_response 403
    ...
    put order_url(@order), params: { order_items: [{ id: @order.order_items.first.id, status: :cancelled }] }, headers: { Authorization: JsonWebToken.generate_token(chef) }, as: :json
    assert_response 403
  ...
  test "update should return forbidden if current_user is waiter and status parameter is completed" do
    waiter = users(:waiter)
    put order_url(@order), params: { order_items: [{ id: @order.order_items.first.id, status: :completed }] }, headers: { Authorization: JsonWebToken.generate_token(waiter) }, as: :json
    assert_response 403
  ...
  test "update should return not_found if order_id is invalid" do
    put order_url(0), params: { is_active: @order.is_active, table_no: @order.table_no }, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    assert_response 404
    assert_json_match(error_response(RECORD_NOT_FOUND), response.body)
  end
  ...
  test "update should update order and order_items" do
    ...
    assert_difference('OrderItem.count') do
      put order_url(@order), params: payload, headers: { 'Authorization': JsonWebToken.generate_token(@user) }, as: :json
    end
